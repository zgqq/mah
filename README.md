# mah
mah 是一个类似于macOS的alfred的软件，纯java编写的，主要针对linux 开发的，可能在windows
和macOS 能够正确运行，但是我没有测试过。
...
# 插件
## 异步执行
所有的命令都是异步执行的，这样你可以同时执行多条命令，比如，我执行命令同步github 已经star的仓库的时候，因为同步的过程可能有点久，有点蛋疼，你可以输入其他命令做其他事，查查单词什么的，不会受影响

## 开发
插件只能通过java编写，具体编写方法，我实在没有精力写什么教程之类的东西，你可以参考 [这几个插件](./mah-plugin) 的写法，但是插件机制感觉还不是很成熟，暂时不建议自已写插件

# 软件缺陷
## 快捷键 
全局快捷键只能监听按键，并不能消耗那个按键的作用，比如你在配置文件里面配置Ctrl+i为弹出软件，但是一个键击同样可以被其他软件接受到,所以你必须让桌面环境消耗掉那个按键，例如窗口管理器i3 加入bindsym Mod1+space exec '' 这样Alt+space就被桌面环境消耗了，其他软件就接受不到这个键击了，也就不会做其他动作了。
...
# 如何使用？
## 基本使用
首先要配置好jdk1.8的环境，然后到那个存放jar包的目录下，执行命令java -jar mah.jar 软件也就启动了

## 一步一步配置
### 快捷键
首先用编辑器打开 /home/用户名/.config/mah/conf.xml 加入
